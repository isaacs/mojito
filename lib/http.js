...
function makeTest (test) {
  ...
  if (typeof(test) === "string") {
    var str = test;
    test = function (w) { return w.request.url.indexOf(str) === 0 && [str] };
  } else if (test instanceof RegExp) {
    var reg = test;
    test = function (w) { return reg.exec(w.request.url) };
  }
  
  // called from an HTTPWalker
  return function (w) {
    var match = test(w);
    ...
    var originalUrl = w.request.url,
      originalMatch = w.match,
      originalMount = w.mountPoint;
    w.match = match;
    if (originalUrl.indexOf(str) === 0) {
      // url mounting
      w.request.url = w.request.url.substr(str.length);
      w.mountPoint = str;
    }
    
    w.current.action.mo.push({action:routeCompleter(originalUrl, originalMount, originalMatch)});
    test = null;
    return true;
  };
};

function routeCompleter (originalUrl, originalMount, originalMatch) { return function routeComplete (w) {
  w.request.url = originalUrl;
  w.mountPoint = originalMount;
  w.match = originalMatch;
  w.next();
}};
...
function HTTPWalker (ji, request, response) {
  if (!(this instanceof HTTPWalker)) {
    return new HTTPWalker(ji, request, response);
  }
...
HTTPWalker.prototype = process.mixin(Object.create(Walker.prototype), aop, {
  ...
  sendHeader : function HTTPWalker_sendHeader (statusCode, headers, now) {
  ...
  readBody : function HTTPWalker_readBody (reader) {
...
aop.after("done", function HTTPWalker_afterDone () {
  return this.finish.apply(this, arguments)
}, HTTPWalker.prototype);

["setBodyEncoding", "pause", "resume"].forEach(function (f) {
  HTTPWalker.prototype[f] = function HTTPWalker_proxyFn () {
    this.request[f].apply(this.request, arguments);
    return this;
  };
});
