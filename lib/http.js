...
function makeTest (test) {
  ...
  return function () {
    ...
    this.current.action.mo.push({ action : function routeComplete (w) {
      w.request.url = originalUrl;
      if (originalMount === undefined) delete w.mountPoint;
      else w.mountPoint = originalMount;
      if (originalMatch === undefined) delete w.match;
      else w.match = originalMatch;
      w.next();
    }});
    return true;
  };
