...
Walker.prototype = {
  // finish the request and terminate the list.
  ...
  sendBody : function (body, encoding) {
    ...
    if (this.request.method !== "HEAD") this.response.sendBody(String(body), encoding || "binary");
    return this;
  },
  sendHeader : function (status, header, now) {
    ...
    if (now) {
      ...
      this.response.sendHeader(this.status || 200, this.header || {});
    }
    
    return this;
  },
  flush : function () { this.response.flush(); return this; },
...
function priv (cw) {
  ...
  if (!p || (p.cw !== cw)) throw new Error(
    "Invalid Walker: " + cw._id + ". Maybe calling next() after finish()?"
  );
  return p;
}
